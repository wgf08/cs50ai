...
def load_data(filename):
    ...
    month = {a: b for b, a in month_num}
...
def evaluate(labels, predictions):
    ...
    """
    sensitivity_guessed = 0
    sensitivity_total = 0
    specificity_guessed = 0
    specificity_total = 0
    
    for i,label in enumerate(labels):
        if label == 1:
            if label == predictions[i]:
                sensitivity_guessed += 1
            sensitivity_total += 1
        if label == 0:
            if label == predictions[i]:
                specificity_guessed += 1
            specificity_total += 1
    return (sensitivity_guessed/sensitivity_total, specificity_guessed/specificity_total)
